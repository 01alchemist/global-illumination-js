...
export class QuadMesh implements PrimitiveList {
    ...
    writeObj(filename:string) {
        try {
            ...
            for (let i:number = 0; (i < this.points.length); i += 3) {
                fileData += "v " + this.points[i] + " " + this.points[(i + 1)] + " " + this.points[(i + 2)] + "\n";
            }

            fileData += "s off\n";
            for (let i:number = 0; (i < this.quads.length); i += 4) {
                fileData += "f " + this.quads[i] + 1 + " " + this.quads[(i + 1)] + 1 + " " + this.quads[(i + 2)] + 1 + " " + this.quads[(i + 3)] + 1 + "\n";
            }
        }
        catch (e) {
            console.log(e);
    ...
    update(pl:ParameterList, api:GlobalIlluminationAPI):boolean {
        ...
        if ((this.quads == null)) {
            console.error("Unable to update mesh - quad indices are missing");
            return false;
        }

        if (((this.quads.length % 4)
            != 0)) {
            console.warn("Quad index data is not a multiple of 4 - some quads may be missing");
        }
        ...
        if (points == null) {
            console.error("Unabled to update mesh - vertices are missing");
            return false;
        }
        ...
        * (quads.length / 4)));
        ...
        if (((faceShaders != null)
            ...
            for (let i:number = 0; (i < faceShaders.length); i++) {
                ...
                if ((v > 255)) {
                    console.warn("Shader index too large on quad %d", i);
                }
