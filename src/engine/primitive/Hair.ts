...
export class Hair implements PrimitiveList, Shader {
    ...
    public update(pl: ParameterList, api: SunflowAPI): boolean {
        ...
        if ((this.numSegments < 1)) {
            UI.printError(Module.HAIR, "Invalid number of segments: %d", this.numSegments);
            return false;
        }
        ...
        if ((pointsP != null)) {
            if ((pointsP.interp != InterpolationType.VERTEX)) {
                UI.printError(Module.HAIR, "Point interpolation type must be set to \""vertex\"" - was \""%s\"""", pointsP.interp.name().toLowerCase())", else, {, points=pointsP.data);
            }
