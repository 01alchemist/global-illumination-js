...
export class TriangleMesh implements PrimitiveList {
    ...
    update(pl:ParameterList, api:GlobalIlluminationAPI):boolean {
        ...
        if ((this.triangles == null)) {
            console.error(Module.GEOM, "Unable to update mesh - triangle indices are missing");
            return false;
        }

        if (((this.triangles.length % 3)
            != 0)) {
            console.warn(Module.GEOM, "Triangle index data is not a multiple of 3 - triangles may be missing");
        }
...
if (((faceShaders != null)
    ...
    for (let i:number = 0; (i < faceShaders.length); i++) {
        ...
        if ((v > 255)) {
            console.warn(Module.GEOM, "Shader index too large on triangle %d", i);
        }
...
intersectPrimitiveRobust(r:Ray, primID:number, state:IntersectionState) {
...
init() {
    ...
    if (!smallTriangles) {
        ...
        if ((nt > 2000000)) {
            console.warn(Module.GEOM, "TRI - Too many triangles -- triaccel generation skipped");
            return;
        }
...
getPoint(tri:number, i:number, p:Point3) {
...
getBakingPrimitives():PrimitiveList {
    switch (uvs.interp) {
        case NONE:
        case FACE:
            console.error(Module.GEOM, "Cannot generate baking surface without texture coordinate data");
            return null;
            break;
        default:
