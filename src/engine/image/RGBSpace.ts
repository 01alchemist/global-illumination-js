...
export class RGBSpace {
    ...
    convertXYZtoRGB(X:number, Y:number, Z:number):Color {
    ...
    insideGamut(r:number, g:number, b:number):boolean {
        return ((r >= 0)
        && ((g >= 0)
        && (b >= 0)));
    }

    gammaCorrect(v:number):number {
        if ((v <= 0)) {
            return 0;
        }
        else if ((v >= 1)) {
    ...
    ungammaCorrect(vp:number):number {
        if ((vp <= 0)) {
            return 0;
        }
        else if ((vp >= 1)) {
    ...
    rgbToNonLinear(rgb:number):number {
