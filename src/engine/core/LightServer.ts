...
import {CausticPhotonMapInterface} from "./CausticPhotonMapInterface";
import {GIEngine} from "./GIEngine";
import {CausticPhotonMap} from "../renderer/photomap/CausticPhotonMap";
import {GIEngineFactory} from "../renderer/gi/GIEngineFactory";
...
export class LightServer {
    ...
    build(options:Options):boolean {
        ...
        if (caustics == null || caustics == "none") {
            ...
            console.warn("Unrecognized caustics photon map engine " + caustics + " - ignoring");
            this.causticPhotonMap = null;
        }
        ...
        console.log("Light Server stats:");
        console.log("  * Light sources found:" + this.lights.length);
        console.log("  * Light samples:      " + numLightSamples);
        console.log("  * Max raytrace depth:");
        console.log("      - Diffuse          " + this.maxDiffuseDepth);
        console.log("      - Reflection       " + this.maxReflectionDepth);
        console.log("      - Refraction       " + this.maxRefractionDepth);
        console.log("  * GI engine            " + options.getString("gi.engine", "none"));
        console.log("  * Caustics:           " + caustics == null ? "none" : caustics);
        console.log("  * Shader override:    " + this.shaderOverride);
        console.log("  * Photon override:    " + this.shaderOverridePhotons);
        console.log("  * Shading cache:      " + shadingCache == null ? "off" : "on");
        console.log("  * Build time:         " + t);
        return true;
    }
    ...
    showStats() {
        ...
        this.shadingCache.forEach(function(e:CacheEntry) {
            numUsedEntries += (e != null) ? 1 : 0;
        });

        // TODO:Warning!!!, inline IF is not supported ?
        console.log("Shading cache stats:");
        console.log("  * Used entries:       %d (%d%%)", numUsedEntries, ((100 * numUsedEntries)
        / this.shadingCache.length));
        console.log("  * Lookups:            %d", this.cacheLookups);
        console.log("  * Hits:               %d", this.cacheHits);
        console.log("  * Hit rate:           %d%%", ((100 * this.cacheHits)
        / this.cacheLookups));
        console.log("  * Empty entry misses: %d", this.cacheEmptyEntryMisses);
        console.log("  * Wrong entry misses: %d", this.cacheWrongEntryMisses);
        console.log("  * Entry adds:         %d", this.cacheEntryAdditions);
    }
    ...
    calculatePhotons(map:PhotonStore, type:string, seed:number):boolean {
        ...
        console.log("Tracing %s photons ...", type);
        ...
        photonTimer.end();
        UI.taskStop();
        console.log("Tracing time for %s photons:%s", type, photonTimer.toString());
        map.init();
        return true;
    }
